...
// pair<ll,ll> solve2(ll times, vll &arr,ll lastweight,ll available, ll x, vpll &dp){

//     if(available==0){
//         return {times,lastweight};
//     }

//     if(dp[available].first!=-1){
//         return dp[available];
//     }
...
//             auto tempx = (1<<i) ^ available;
//             pll temp;

//             if(lastweight + arr[i] > x)
//                 temp = solve2(times+1,arr,arr[i],tempx,x,dp);
            
//             else
//                 temp = solve2(times,arr,lastweight + arr[i], tempx,x, dp);

//             if((temp.first < ans.first) || (temp.first == ans.first and temp.second < ans.second)){
//                 ans = temp;
//             }
//         }
//     }

//     return dp[available] = ans;
// }
    ...
    for(int mask = 1;mask<(1<<n);mask++){
        for(int bit = 0;bit < n; bit++){
            if((1<<bit)&mask){
                ...
                if(space_taken + arr[bit] <= x){
                    dp[mask] = min(dp[mask],{dp[mask^(1<<bit)].first, space_taken + arr[bit]});
                }

                else{
                dp[mask] = min(dp[mask],{1  + dp[mask^(1<<bit)].first,arr[bit]});
                }
            }
