...
void solve() {
    ...
    f(i,0,n){
        f(j,i,n){
            int maxi = max(max(mat[i][j],mat[j][n-1-i]),max(mat[n-j-1][i],mat[n-i-1][n-1-j]));
            count += (abs(maxi - mat[i][j]) + abs(maxi - mat[j][n-1-i]) + abs(maxi - mat[n-j-1][i]) + abs(maxi - mat[n-1-i][n-1-j]));
            mat[i][j] = maxi;
            mat[j][n-1-i] = maxi;
            mat[n-j-1][i] = maxi;
            mat[n-1-i][n-1-j] = maxi;
        }
    }
