...
void solve2(int ind, int lastdigit, string temp, string &num, bool flag, ll  &ans){
    ...
    for(int digits = 0;digits<=limit;digits++){
        ...
        if(lastdigit==-1 || digits!=lastdigit){

            if(digits<limit)
                solve2(ind+1,digits,temp + to_string(digits), num,true, ans);

            else
                solve2(ind+1,digits,temp + to_string(digits) ,num,false, ans);
        }
    }
    // return ans;
}
...
void solve() {
   ...
   ll ans1 = 0,ans2 = 0;
    ...
    solve2(0,-1,"",num1,false,ans1);
    solve2(0,-1,"",num2,false,ans2);
    cout << ans2 - ans1 << endl;
}
