...
bool isPossible(vll &v, int i, int ang){
    if(i == v.size())   return ang == 0;
    return isPossible(v, i+1, (ang+v[i])%360) || isPossible(v, i+1, (360+ang-v[i])%360);
}

void solve() {
    in(n);  inarr(n, v);
    if(isPossible(v, 0, 0)) pyes
    else    pno
}
