...
void dfs(ll i,ll j, vvll &vis, vector<vector<char>> &building){

    vis[i][j] = 1;
    for(auto it : dir4){
        ll tempx = it.first + i;
        ll tempy = it.second + j;

        if(tempx>=0 && tempx<building.size() && tempy>=0 && tempy<building[0].size() && !vis[tempx][tempy] && building[tempx][tempy]=='.'){
            dfs(tempx,tempy,vis,building);
        }
    }
}
