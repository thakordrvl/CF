...
ll solve2(int ind, string &s, map<string,bool> &ispresent,vll &dp){
    ...
    for(int i=ind;i<s.size();i++){
        ...
        auto str = s.substr(ind,i-ind+1);

        if(ispresent[str]){
            // dbg1(str)
            ans += solve2(i+1,s,ispresent,dp);
            ans = ans % mod;
        }
    }
...
//    in(n);
   ...
   ll n = s.size();
    ...
    cout << solve2(0,s,ispresent,dp) << endl;
}
