...
void solve() {
    ...
    if(n&1){
        ...
        for(auto it : isbit){
            if(it.second&1){
                ansmaxi = ansmaxi | 1<<it.first;
            }

            // dbg2(it.first,it.second)
        }

        cout << xori << " " << ansmaxi << endl;
        // dbans(xori)
        // dbans(ansmaxi)
    }
        ...
        ll mini = 0;
        // cout << bitset<8>(maxi).to_string() << endl;
        // cout << bitset<8>(xori).to_string() << endl;


        for(int i=0;i<=63;i++){
            if(!((maxi>>i)&1) & ((xori>>i)&1)){
                mini = mini | 1<<i;
            }
        }

        // cout << bitset<8>(mini).to_string() << endl;
        cout << mini << " " << xori << endl;
        // dbans(mini)
        // dbans(xori)
    }
