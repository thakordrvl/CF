...
#define inarr(n, arr)   vector<int> arr(n); for(int i=0; i<n; i++) cin >> arr[i]
...
void solve() {
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=0;j<=x;j++){
            ...
            curr[j]=max(curr[j],(j-prices[i]>=0?books[i] + prev[j-prices[i]]:0));
            // if(i>=prices[j])
            //     temp = max(temp,books[j] + prev[i-prices[j]]);
        ...
        // for(auto it: curr) cout<<it<<" "; cout<<endl;
        prev = curr;
    }
